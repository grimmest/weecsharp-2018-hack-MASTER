...
def getHeading(x1,x2,y1,y2, heading=0):
...
while True:
	...
	if i == 0:
		...
		y=message.get('Y')
		print(y)
		heading_to_origin=getHeading(0.0,x,0.0,y)
		logging.info("Moving to origin")
		GameServer.sendMessage(ServerMessageTypes.MOVEFORWARDDISTANCE, {'Amount': heading_to_origin})

		if (message.get('Name')=='RandomBot'):
			if (message.get('X')>-1 and message.get('X')<) 
				logging.info("Moving randomly")
				GameServer.sendMessage(ServerMessageTypes.MOVEFORWARDDISTANCE, {'Amount': random.randint(0, 5)})
'''
...
'''
	...
	try:
		if (message['Name']=='RandomBot') and  (message['Health']<3): #having problems when getting hit or when picking up objects
			...
			if message['Type']=='HealthPickup':
				x2=message['X']
				y2=message['Y']
				print(getHeading(x1,x2,y1,y2))
			#print("this ran")
	except:
		print("error kicked up")
